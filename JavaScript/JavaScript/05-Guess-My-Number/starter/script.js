...
//Create a highsore 
let highsore = 0;
//Create a function to display the message for each condition
...
//Create a function to display the number 
...
document.querySelector(".check").addEventListener('click',function(){
    ...
    }//When the player guess incorrect 
    else if(guess !== Correct_Number){
        if (score > lose_point){
            // use a turnery operator
            // ? mean if
            // : mean else
            //document.querySelector('.message').textContent = guess > Correct_Number ? 'Too hight' : "Too low";
            displayMessage(guess > Correct_Number ? 'Too hight' : "Too low");
            // Guess worng number - 1 score;
            score --; // => score = score - 1;
            // Display to score 
            displayScore(score);
